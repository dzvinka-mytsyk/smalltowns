...
(function() {
  ...
  var optimizeWaypoints = true; //added by Dzvinka
  ...
  var originalOnFatalErrorCallback = function(tsp, errMsg) {
    if (window && window['console']) console.log("Request failed: " + errMsg);
  }
  ...
  function directions(mode) {
    ...
    if (numActive > maxTspSize) {
      if (window && window['console']) console.log("Too many locations! You have " + numActive + ", but max limit is " + maxTspSize);
    } else {
      legsTmp = new Array();
      distances = new Array();
      durations = new Array();
  ...
  function nextChunk(mode) {
    //  if (window && window['console']) console.log("nextChunk");
      ...
      myGebDirections.route({
        ...
        function(directionsResult, directionsStatus) {
          if (directionsStatus == google.maps.DirectionsStatus.OK) {
            ...
            //if (window && window['console']) console.log("Request completed!");
            ...
            if (window && window['console']) console.log("Request failed: " + errorMsg);
          }
        });
    } else {
      readyTsp(mode);
  ...
  function readyTsp(mode) {
    //if (window && window['console']) console.log("readyTsp");
  ...
  function doTsp(mode) {
    //if (window && window['console']) console.log("doTsp");
  ...
  function addAddress(address, label, callback) {
    ...
    gebGeocoder.geocode({
      ...
      if (status == google.maps.GeocoderStatus.OK) {
        ...
        --addressRequests;
        if (window && window['console']) console.log("Failed to geocode address: " + address + ". Reason: " + GEO_STATUS_MSG[status]);
        ++currQueueNum;
        addressProcessing = false;
        if (typeof(callback) == 'function')
          callback(address);
  ...
  function BpTspSolver(map, panel, onFatalError) {
    if (tsp) {
      if (window && window['console']) console.log('You can only create one BpTspSolver at a time.');
      return;
    }
  ...
  BpTspSolver.prototype.solveRoundTrip = function(callback) {
    if (doNotContinue) {
      if (window && window['console']) console.log('Cannot continue after fatal errors.');
      return;
    }
